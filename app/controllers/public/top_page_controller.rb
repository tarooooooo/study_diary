  ...
  def show
    ...
    if current_user&.study_categories
      current_user.study_categories.each_with_index  do | study_category, number |
        @weekly_study_time << ["#{Date.current - number}", study_category.learning_diaries.where(study_day: (Date.current - number)).sum(:study_time) ]
      end

      current_user.study_categories.each do | study_category |
        @learning_diaries_chart_data.push({ name: study_category.name, data: @weekly_study_time })
      end

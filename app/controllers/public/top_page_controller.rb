    ...
    if user_signed_in?
      ...
      if current_user.learning_diaries
        ...
        days.each_with_index do | day, number_of_day |
          uncategorized_diary_day_and_study_times.push(["#{day}", current_user.learning_diaries.weekly_uncategorized_diaries(number_of_day).sum(:study_time) ])
        end
        @learning_diaries_chart_data.push({ name: "カテゴリ未指定", data: uncategorized_diary_day_and_study_times})

        # カテゴリ別の円チャート用データ作成（カテゴリなし））
        uncategorized_diaries_study_time = current_user.learning_diaries.weekly_uncategorized_diaries(Date.current - Date.today.beginning_of_week).sum(:study_time)
      ...
      if current_user&.study_categories
        ...
        @category_and_study_time_hash = category_and_study_time_hash.merge(current_user.study_categories.joins(:learning_diaries).where(learning_diaries: { study_day: Date.today.beginning_of_week..Date.current }).group(:name).sum("learning_diaries.study_time")).sort_by { |_, v| -v }.to_h
      end

      # 1日の平均学習時間を算出
      weekly_study_time = current_user.weekly_study_time
      @weekly_study_time_hh, @weekly_study_time_mm = weekly_study_time.divmod(60)
      avg_daily_study_time = weekly_study_time / (Date.today.beginning_of_week..Date.current).count
      @avg_study_time_hh, @avg_study_time_mm = avg_daily_study_time.divmod(60)
      ...
      @study_plan_weekly_progress = [@current_study_plan.total_progress, 100].min

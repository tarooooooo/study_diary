class Public::TopPageController < ::Public::BaseController
  ...
  def show
    if user_signed_in?
      ...
      # 1日の平均学習時間を算出
      weekly_study_time = current_user.learning_diaries.where(study_day: Date.today.beginning_of_week..Date.current).sum(:study_time)
      ...
      study_plan_weekly_progress = [( weekly_study_time / @current_study_plan.weekly_target_time.to_f * PERCENT_CONVERTION_NUMBER).floor, 1].max
